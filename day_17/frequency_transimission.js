...
const executeSignal = (input) => {
  let prevElement = 0;

  for (let index = input.length - 1; index >= 0; index--) {
    input[index] = (input[index] + prevElement) % 10;
    prevElement = input[index];
  }
};

const part2 = (data, phase) => {
  const input = data.split("").map(Number);
  const offset = parseInt(input.slice(0, 7).join(""));
  console.log("offset", offset);
  const signal = input.slice(offset);
...
console.log("fft", part2(repeatInput(data), 100));
